    ...
    if (isset($_POST['prov-name'])) {
        if (!empty($_POST['prov-name'])) {
            ...
            if ($img_up_status1 && $img_up_status2) {
                ...
                if(strtolower($_POST['prov-field']) != 'education'){
                    $prov_service = htmlspecialchars($_POST['prov-service']);
                }
                else{
                    $prov_service = htmlspecialchars($_POST['prov-service1']);
                }
                $prov_spec_1 = htmlspecialchars($_POST['prov-spec-1']);
				$prov_spec_2 = htmlspecialchars($_POST['prov-spec-2']);
                $prov_spec_3 = htmlspecialchars($_POST['prov-spec-3']);
				$prov_spec_4 = htmlspecialchars($_POST['prov-spec-4']);
				$prov_workexp = htmlspecialchars($_POST['prov-workexp']);
                $prov_work_record = htmlspecialchars($_POST['prev-working-addr1']).htmlspecialchars($_POST['prev-working-addr1'])
				.htmlspecialchars($_POST['prev-working-addr1']).htmlspecialchars($_POST['prev-working-addr1']).
				htmlspecialchars($_POST['prev-working-addr1']).htmlspecialchars($_POST['prev-working-addr1']);
                $prov_about = htmlspecialchars($_POST['prov-about']);
				$prov_price = htmlspecialchars($_POST['prov-price']);
                $prov_id_front = $hash1 . '.' . $img_ext1;
                $prov_id_bk = $hash2 . '.' . $img_ext2;
                $prov_hash = md5(uniqid(rand(), true));

                $db = new DB();
                $db->query("INSERT INTO `ww_provider` ( `prov_name`, `prov_contact`, `email_id`, `pwd`, `prov_dob`, 
				`prov_gender`, `prov_addr`, `prov_pincode`, `prov_state`, `prov_field`, `prov_service`, 
				`prov_spec_1`, `prov_spec_2`, `prov_spec_3`, `prov_spec_4`, `prov_workexp`, `prov_record`, `prov_price`,
				`prov_about`, `prov_id_front`, `prov_id_bk`, `prov_hash`, `reg_date`, `is_active`) 
				VALUES (:prov_name, :prov_contact, :email_id, :pwd, :prov_dob, :prov_gender, :prov_addr, 
				:prov_pincode, :prov_state, :prov_field, :prov_service, :prov_spec_1, :prov_spec_2, :prov_spec_3, :prov_spec_4, :prov_workexp, 
				:prov_record, :prov_price, :prov_about, :prov_id_front, :prov_id_bk, :prov_hash, now(), :is_active)");
                ...
                $db->bind(':prov_spec_1', $prov_spec_1);
				$db->bind(':prov_spec_2', $prov_spec_2);
				$db->bind(':prov_spec_3', $prov_spec_3);
				$db->bind(':prov_spec_4', $prov_spec_4);
                $db->bind(':prov_workexp', $prov_workexp);
                $db->bind(':prov_record', $prov_work_record);
				$db->bind(':prov_price', $prov_price);
