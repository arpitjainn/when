<header>
		...
		<form style="display:inline-block;width:70%;vertical-align:middle;">
			...
			<select name="speciality3" id="speciality3"  style="width:20%;padding:5px;border-radius:5px;vertical-align:middle;"	>
				<option value="">Select</option>
			</select>
	...
	var extraOptions = {
		...
		//Personal Services Specialities
			...
			var speciality3 = $("select[name=speciality3]");
			function populateSpeciality3(){
				var serviceVal = "<?php echo $profession ?>";
				//speciality2.html('');  // remove all options bar first
				$.each(extraOptions, function (Ind, Val) {
					if (Ind == serviceVal) {
						$.each(Val, function (Ind1, Val1) {
							var optionObj = new Option(Val1, Val1);
							speciality3.append(optionObj);
						});
					}
				});	
			}
