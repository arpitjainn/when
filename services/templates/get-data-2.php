<?php
	...
	if(isset($_GET['search'])){
		...
		$db = new DB();
        $db->query("SELECT `prov_id`,`prov_name`, `prov_gender`, `prov_spec_1`, `prov_spec_2`,
		`prov_service_as`, `prov_workexp`, `base_price`, `prov_about`, `prov_photo` 
		FROM `ww_provider` WHERE `prov_service`='$profession' and `prov_spec_1` like concat('%','$speciality','%')
		and `prov_spec_2` like concat('%','$bestFor','%') ; ");
		...
		$db = new DB();
        $db->query("SELECT `prov_id`, `prov_name`, `prov_gender`, `prov_addr`, `prov_spec_1`, `prov_spec_2`,
		...
		and `prov_spec_1` like concat('%','$speciality','%') and `prov_spec_2` like concat('%','$bestFor','%') 
		and `prov_service_as` like concat('%','$provideServiceAs','%');; ");
        $db->bind(':profession', 'carpenter');
        $exeRes = $db->resultset();
		$count = $db->rowCount();
        $db->terminate();
		...
		$db = new DB();
        $db->query("SELECT `prov_id`, `prov_name`, `prov_gender`, `prov_spec_1`, `prov_spec_2`, `prov_spec_4`, 
		`prov_workexp`, `prov_price`, `prov_about`, `prov_photo`, `discount`, `rating` FROM `ww_provider` 
		WHERE `prov_service`='$profession' ; ");
        $db->bind(':profession', 'carpenter');
        $exeRes = $db->resultset();
		$count = $db->rowCount();
        $db->terminate();
