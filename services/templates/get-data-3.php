<?php
	...
	if(isset($_GET['search'])){
		...
		$bestFor = $_GET['speciality2'];
		$extra = $_GET['speciality3'];
		$db = new DB();
        $db->query("SELECT `prov_id`,`prov_name`, `prov_gender`, `prov_spec_1`, `prov_spec_2`,`prov_spec_3,` ,`prov_spec_4`, `prov_workexp`,
		`prov_price`, `prov_about`, `prov_photo`,`rating`,`discount` FROM `ww_provider` WHERE `prov_service`='$profession' 
		and `prov_spec_1` like concat('%','$speciality','%') and `prov_spec_2` like concat('%','$bestFor','%') and `prov_spec_3` like concat('%','$extra','%') ; ");
		...
		$db = new DB();
        $db->query("SELECT `prov_id`,`prov_name`, `prov_gender`, `prov_addr`, `prov_spec_1`, `prov_spec_2`,`prov_spec_3`,
		`prov_spec_4`, `prov_workexp`, `prov_price`, `prov_about`, `prov_photo`,`rating`,`discount` FROM `ww_provider` 
		WHERE `prov_service`='$profession' and `prov_price`<='$budget' and `prov_addr` like concat('%','$location','%')
		and `prov_spec_1` like concat('%','$speciality','%') and `prov_spec_2` like concat('%','$bestFor','%') and 
		`prov_spec_3` like concat('%','$extra','%') and `prov_spec_4` like concat('%','$provideServiceAs','%');; ");
        $db->bind(':profession', 'carpenter');
        $exeRes = $db->resultset();
		$count = $db->rowCount();
        $db->terminate();
		...
		$db = new DB();
        $db->query("SELECT `prov_id`,`prov_name`, `prov_gender`, `prov_spec_1`, `prov_spec_2`, `prov_spec_3`,`prov_spec_4`,
		`prov_workexp`, `prov_price`, `prov_about`, `prov_photo`,`rating` , `discount` FROM `ww_provider` WHERE `prov_service`='$profession' ; ");
        $db->bind(':profession', 'carpenter');
        $exeRes = $db->resultset();
		$count = $db->rowCount();
        $db->terminate();
