$(document).ready(function(){
		...
		function isEmpty(str){if(str==''||str==' '||str==null)
			return!0}
		...
		$('.oc-2').owlCarousel({
			...
			margin:10,
			nav:!0,
			navText:["<span class='prev-slider-btn'>&lsaquo;&lsaquo;</span>","<span class='next-slider-btn'>&rsaquo;&rsaquo;</span>"],
			autoplay:!0,
	...
	var providerSpeciality = $("select[name=prov-spec-1]");
	var providerBestFor = $("select[name=prov-spec-2]");
	var providerExtra = $("select[name=prov-spec-3]");
	var providerServiceAs = $("select[name=prov-spec-4]");
	providerService.change(function () {
		...
		$('.prov-spec-2').addClass('hidden');
		providerBestFor.html('none');  // remove all options bar first
		$.each(bestForOptions, function (bestForInd, bestForVal) {
            if (bestForInd == serviceVal) {
				$('.prov-spec-2').removeClass('hidden');
				providerBestFor.html('')
		...
		$('.prov-spec-3').addClass('hidden');
		providerExtra.html('none');  // remove all options bar first
		$.each(extraOptions, function (extraInd, extraVal) {
            if (extraInd == serviceVal) {
                $('.prov-spec-3').removeClass('hidden');
				providerExtra.html('');
		...
		$('.prov-spec-4').addClass('hidden');
		providerServiceAs.html('none');  // remove all options bar first
		
		$.each(serviceAsOptions, function (serviceAsInd, serviceAsVal) {
            if (serviceAsInd == serviceVal) {
				$('.prov-spec-4').removeClass('hidden');
				providerServiceAs.html('');
	...
	var providerWorkExperience = $("select[name=prov-workexp]");
	providerWorkExperience.change(function () {
		if(providerWorkExperience.val()=="Fresher"){
			$('.prev-work-record').addClass('hidden');
		}
		else{
			$('.prev-work-record').removeClass('hidden');
		}
	});
	...
	function populateSpeciality1(){
		...
		$(".otp").click(function(){
			...
			if(mobNumLen>=10&&mobNumLen<15){
				var request=$.ajax({url:"/resources/pages/otp-sender.php",method:"POST",data:{mobNum:mobnumber},dataType:"html"});
		...
		$('.signup-next-btn').click(function(){
			...
			if(mOtp.toString().length>=2){
				var request=$.ajax({
					url:"/resources/pages/otp-verify.php",
					method:"POST",
					data:{mobOtp:mOtp},
					dataType:"html"});
				request.done(function(msg){
		...
		$('.signup-prov-btn').click(function(){
			...
			f11=!0;//validateInput("input[name=prov-speciality]",".error-speciality");
			f12=!0;//validateInput("select[name=prov-workexp]",".error-prov-workexp");
			f13=!0;//validateInput("textarea[name=prov-record]",".error-prov-record");
			f14=!0;//validateInput("textarea[name=prov-about]",".error-prov-about");
			f15=!0;//validateImage("input[name=prov-front-id]",".error-prov-front-id");
			f16=!0;//validateImage("input[name=prov-back-id]",".error-prov-back-id");
			if(!$('.transport').hasClass('hidden')){
				fh=!0;fh1=validateInput("select[name=provh-vehicle]",".error-provh-vehicle");
				...
				fh=!0;fh1=validateInput("input[name=provh-degree]",".error-provh-degree");
		...
		$(".detect-btn").click(function(){
			...
			if(window.navigator.geolocation){
				navigator.geolocation.getCurrentPosition(function(position){
					var lat=position.coords.latitude,lng=position.coords.longitude,latlng=new google.maps.LatLng(lat,lng),geocoder=new google.maps.Geocoder();
					geocoder.geocode({'latLng':latlng},function(results,status){
						if(status==google.maps.GeocoderStatus.OK){
							if(results[1]){
								...
								console.log("Geocoder failed: "+status)}})},function(){
